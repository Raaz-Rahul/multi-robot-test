...
# Multi-robot load-sharing environment 
...
class MultiRobotEnv:
    ...
    Actions (per robot): [move, offload, partner, meeting_idx, w_idx]
    """
    ...
    def reset(self, seed: Optional[int] = None):
        ...
        # initial positions on row 0, spread columns
    ...
    # Movement helpers
    # -------------------------
    def _apply_move(self, pos, move, robot_idx=None):
    ...
    def _compute_energy_forecast(self, robot_idx, pos_override=None):
    ...
    def _step_towards(self, pos, target):
        x, y = pos
        tx, ty = target
    ...
    def step(self, action: np.ndarray):
        ...
        Action encoding: per-robot block [move, offload, partner, meeting_idx, w_idx]
        """
        if self.done:
        ...
        # 3) sharing proposals (validate and collect)
        proposals = []
        for i in range(self.n):
            if parsed[i]['offload'] == 1 and self.batteries[i] > 0:
                j = parsed[i]['partner']
                if j == i: continue
                if self.batteries.get(j, -1) <= 0: continue
        ...
        # 4) resolve sharing (one-step move toward meeting node, transfer only when co-located)
        ...
        for i in range(self.n):
            ...
            self.batteries[i] -= e
            total_energy += e
            if parsed[i]['move'] != 0 and prev_positions[i] == self.positions[i]:
                penalty -= 1
            if i in collisions:
                penalty -= 5
        ...
        obs = self._get_obs()
        info = {"energy": total_energy, "batteries": dict(self.batteries)}
