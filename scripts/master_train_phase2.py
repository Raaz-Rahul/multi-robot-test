...
# Unified pipeline: Demos → BC → PPO (Colab & VS Code compatible)
# ==============================================================

import os, sys, numpy as np, torch
from tqdm import tqdm

# --- Path fix (works in Colab & VS Code) ---
...
def generate_demos(n_episodes=200, max_steps=150, save_path="data/demos.npz"):
    ...
    for ep in tqdm(range(n_episodes), desc="Generating demos"):
        ...
        while not done:
            action = env.action_space.sample().tolist()  # random actions
...
# ==============================================================
# 3️⃣ Phase 3 - PPO Warm-start Training
# ==============================================================
def train_ppo(bc_path="models/bc_policy.pt", total_timesteps=100000, out_path="models/ppo_multi_robot.zip"):
    ...
    env = make_vec_env(lambda: MultiRobotEnergyEnv(n_robots=3, grid_size=5, battery=50, debug=False), n_envs=4)
    model = PPO("MlpPolicy", env, verbose=1)

    # Warm-start with BC weights if available
